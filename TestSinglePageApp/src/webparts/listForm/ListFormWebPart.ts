...
export default class ListFormWebPart extends BaseClientSideWebPart<IListFormWebPartProps> {
  ...
  public render(): void {
    ...
    let itemId:number = 1;
    let FormMode:number = 3;
    let ListUrl: string = "/sites/Contract-Spfx/Lists/ContractData";
    // if item Id is not a number we assume it is a query string parameter
    const urlParams = new URLSearchParams(window.location.search);
    if(urlParams.get("Mode") != null){
      FormMode = Number(urlParams.get("Mode"));
    }
    if(urlParams.get("ID") != null){
      itemId = Number(urlParams.get("ID"));
    }
    if(urlParams.get("ListUrl") != null){
      ListUrl = urlParams.get("ListUrl");
    }
    ...
    } else if (ListUrl != "") {//this.properties.listUrl
      console.log("FormType",this.properties.formType)
      console.log("itemId",itemId)
      // show actual list form react component
      element = React.createElement(
        ListForm,
        {
          inDesignMode: false,//this.displayMode === DisplayMode.Edit
          spHttpClient: this.context.spHttpClient,
          title: this.properties.title,
          description: this.properties.description,
          webUrl: this.context.pageContext.web.absoluteUrl,
          listUrl: ListUrl,//this.properties.listUrl,
          formType: FormMode,//this.properties.formType,
          id: itemId,
          ...
          if (!this.cachedLists) {
            return this.listService.getListsFromWeb(this.context.pageContext.web.absoluteUrl)
              .then((lists) => {
                ...
                console.log("cachedLists",this.cachedLists)
              });
          } else {
