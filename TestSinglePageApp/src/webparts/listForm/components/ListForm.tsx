...
require('../../testSinglePageApp/components/SideNav/master.js');
      ...
      <div className='ard-formFieldsContainer' >
        {
          fields.map((field, idx) => {
            ...
            if (fieldSchemas.length > 0 && fieldSchemas[0].InternalName != "Attachments") {
              ...
              return <div className={fieldSchemas[0].InternalName}>{fieldComponent}</div>;
              }
            }
          })
        }
...
public closemdl():void{
  console.log("formdata",this.state);
  this.props.onSubmitSucceeded(0);
}
  public componentDidMount(): void {
    this.readSchema(this.props.listUrl, this.props.formType).then(
      () => this.readData(this.props.listUrl, this.props.formType, this.props.id).then(()=>{
  ...
  public componentDidUpdate(prevProps, prevState, snapshot):void{
    ...
    if(this.props.title == "Contract Type"){
      console.log("Test-1");
      this.contracttypescriptload();
          }
  }
...
this.contracttypescriptload();
  ...
  public contracttypescriptload():void{
    console.log("contracttypescriptload")
    let ContractAdminRequiredVal = this.state.data["ContractAdminRequired"];
    let ContractAdminVal = this.state.data["ContractAdministrator"];
    let ContractspecVal = this.state.data["Contract_x0020_Specialist"];
    
    console.log("ContractAdminRequiredVal",ContractAdminRequiredVal)
if(ContractAdminRequiredVal == "Yes"){
$('.Contract_x0020_Specialist').hide();
$('.ContractAdministrator').show();
}else{
  $('.Contract_x0020_Specialist').show();
$('.ContractAdministrator').hide();
}

  }

  public contracttypescriptvalidate = () => {
    console.log("contracttypescriptvalidate")
    let ContractAdminRequiredVal = this.state.data["ContractAdminRequired"];
    let ContractAdminVal = this.state.data["ContractAdministrator"];
    let ContractspecVal = this.state.data["Contract_x0020_Specialist"];
    let CTypetitle = this.state.data["Title"];
    let fieldErrorsnew = {};
    console.log("ContractAdminRequiredVal",ContractAdminRequiredVal)
if(ContractAdminRequiredVal == "Yes"){
  console.log("Err-1");
  let fieldnmes:string = "ContractAdministrator";
if(ContractAdminVal == null || ContractAdminVal == "" || ContractAdminVal.length == 0){
  
  console.log("Err-2");
  fieldErrorsnew = {
    ...fieldErrorsnew,
    [fieldnmes]: "Please enter a value!"
  };
  console.log("Err-22",fieldErrorsnew);
}
}else{
  console.log("Err-3");
  let fieldnme:string = "Contract_x0020_Specialist";
  if(ContractspecVal == null || ContractspecVal == "" || ContractspecVal.length == 0){
    
    console.log("Err-4");
    fieldErrorsnew = {
      ...fieldErrorsnew,
      [fieldnme]: "Please enter a value!"
    };
    console.log("Err-22",fieldErrorsnew);
  }
}
if(CTypetitle == null || CTypetitle == ""){
  let fieldnme:string = "Title";
  console.log("Err-40");
  fieldErrorsnew = {
    ...fieldErrorsnew,
    [fieldnme]: "Please enter a value!"
...
this.setState({
  fieldErrors: fieldErrorsnew
});
console.log("fieldErrors",this.state.fieldErrors)

for (let key in fieldErrorsnew) {
  if (fieldErrorsnew[key]) {
    return false;
  }
}
return true;
  ...
  private validator = () => {
    ...
    console.log("validator",this.state.fieldErrors)
  ...
  private async saveItem(): Promise<void> {
   // this.contracttypescriptvalidate();
    let shouldSave = this.props.title=="Contract Type"? this.contracttypescriptvalidate():this.validator();//this.validator();
    if (shouldSave) {
